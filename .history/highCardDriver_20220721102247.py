        ...
        while action != -1:
            if action == 0: # smallBlind's turn
                ...
                if myDecision[0] == "Fold":
                    ...
                    smallBlind.setMoney(smallBlind.getMoney() + smallBlind.getMoneyInPot() - raiseAmount)
                ...
                if myDecision[0] == "Fold":
                    ...
                    bigBlind.setMoney(bigBlind.getMoney() + bigBlind.getMoneyInPot() - raiseAmount)
...
def getFirstValidAction(player, actions, betSize):
    listActions = actions
    #sprint(listActions)
    sortedActions = torch.sort(actions, descending=True)
    #print(sortedActions)
    sortedActions = sortedActions.toList()
    print(sortedActions)
    print(type(sortedActions))
    for i in range(0, len(actions)):
        print(sortedActions[0][0][i])
        print(type(sortedActions[0][0][i]))
        possibleAction = listActions[sortedActions[i]]
